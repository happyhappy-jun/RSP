...
class JhmdbSet(torch.utils.data.Dataset):
    def __init__(self, args, sigma=0.5):
        ...
        self.imgSize = 320
        self.patch_size = 16
        self.videoLen = args.n_last_frames
        ...
        for line in f:
            ...
            lblfile = rows[0]
            jpgfile = rows[1]
            self.jpgfiles.append(os.path.join(args.root, jpgfile))
            self.lblfiles.append(os.path.join(args.root, lblfile))
    ...
    def __getitem__(self, index):
        ...
        for i in range(frame_num):
            ...
            if self.imgSize > 0:
                ...
                img = resize(img, self.imgSize, self.imgSize)
                newh = self.imgSize
                neww = self.imgSize
        ...
        rsz_h, rsz_w = img.size(1) // self.patch_size, img.size(2) // self.patch_size
        ...
        return imgs, imgs_orig, lbls_resize, lblset
