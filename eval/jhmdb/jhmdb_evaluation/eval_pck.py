...
if __name__ == '__main__':
    ...
    parser.add_argument('--filelist', type=str, default='/data/JHMDB/val_list.txt')
    ...
    for cnt, line in enumerate(f):
        ...
        lbls_mat = sio.loadmat(os.path.join(args.root, lblpath))
        ...
        img = cv2.imread(os.path.join(args.root, imgpath))
    ...
    for i in range(len(gts)):
        predfile = args.src_folder + '/' + str(i) + '.dat'
    ...
    os.makedirs(f'{args.src_folder}/pck', exist_ok=True)
    f = open(f'{args.src_folder}/pck/output.txt', 'w')
    for i in range(len(rng)):
        pckall = computePCK(distAll, rng[i])
        print(str(rng[i]) + ': ' + str(pckall[-1]) )
        f.write(str(rng[i]) + ': ' + str(pckall[-1]))
        # print(pckall[:-1])
        # print(pckall[-1])
