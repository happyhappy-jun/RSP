...
Some parts are taken from https://github.com/facebookresearch/dino/blob/main/eval_video_segmentation.py
"""
import os
import copy
import glob
...
def eval_video_tracking_davis(
    args, model, frame_list, video_dir, first_seg, seg_ori, first_frame_idx, last_frame_idx, color_palette
):
    """
    ...
    # first frame
    frame1, ori_h, ori_w = read_frame(frame_list[first_frame_idx])
    ...
    for cnt in range(first_frame_idx + 1, last_frame_idx + 1):
...
def read_frame(frame_dir, scale_size=[560]):
    """
...
def read_seg(seg_dir, factor, scale_size=[560]):
    seg = Image.open(seg_dir)
    _tw, _th = seg.size  # note PIL.Image.Image's size is (w, h)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Evaluation with video object segmentation on VIP"
    )
    ...
    parser.add_argument("--data_path", default="/data/DAVIS", type=str)
    parser.add_argument(
    ...
    parser.add_argument(
        "--bs", type=int, default=64, help="Batch size, try to reduce if OOM"
    )
    parser.add_argument(
    ...
    video_list = open(
        os.path.join(args.data_path, "val_videos.txt")
    ).readlines()
    for i, video_name in enumerate(video_list):
        video_name = video_name.strip()
        print(f"[{i}/{len(video_list)}] Begin to segmentate {video_name}.")#, end='\r')
        video_dir = os.path.join(args.data_path, "Images", video_name)
        frame_list = read_frame_list(video_dir)
        first_frame_idx = 0
        last_frame_idx  = 0
        while True:
            seg_path = (
                frame_list[first_frame_idx].replace("Images", "Annotations").replace("jpg", "png")
            )
        ...
        for i in range(len(frame_list)):
            if os.path.exists(frame_list[first_frame_idx].replace("Images", "Annotations").replace("jpg", "png")):
                last_frame_idx = i
