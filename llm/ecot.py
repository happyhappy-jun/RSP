...
def save_dataset_as_npy(dataset, output_dir="/root/RSP/demo/npy_dataset"):
    os.makedirs(output_dir, exist_ok=True)
    
    for sample_idx, traj in tqdm(enumerate(dataset), desc="Saving to NPY"):
        # Extract observation data
        observation = traj["observation"]["image_image_0"].numpy()
        
        # Attempt to decode move data using utf-8
        try:
            moves = [move.decode('utf-8', 'ignore') for move in traj["move"].numpy()]
        except Exception as e:
            print(f"Error decoding move data at index {sample_idx}: {e}")
            continue
        ...
        try:
            ...
            print(f"Error decoding file_name at index {sample_idx}: {e}")
            continue
        
        # Clean and prepare a path for this trajectory
        ...
        # Create a directory for this trajectory
        traj_dir = os.path.join(output_dir, traj_name)
        os.makedirs(traj_dir, exist_ok=True)

        # Save images and moves as separate .npy files
        np.save(os.path.join(traj_dir, f"images_traj{sample_idx:06d}.npy"), observation)
        np.save(os.path.join(traj_dir, f"moves_traj{sample_idx:06d}.npy"), moves)

    print(f"Saved {sample_idx + 1} trajectories into NPY format at {output_dir}")

save_dataset_as_npy(dataset)